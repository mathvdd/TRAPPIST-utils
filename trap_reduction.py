...
filt_list = ['OH','CN','C2','C3','NH','CO+','H2O','UC','BC','RC','GC','R','I', 'B', 'V','NaI','Ic','Rc'] #to be coherent with the subsets file for progtrap2.cl
...
def generate_ZP_new(calib_dir, obs, date, filtlist, ZPparams=ZP, output_dir=None):
    ...
    if obs == 'TN':
        ...
        ZPtable = pd.read_csv(os.path.join(calib_dir, "median_ZPC_South.log"), header=None, sep="\s+")
    else:
        print('WARNING: observatory not well defined')
        warning_flag = True

    #format the imported table columns
    ZPtable = ZPtable.drop([2,4,5,8,9,10,12,14,16], axis=1)
    ZPtable.columns = ['filt','fwslot','lambda','JDstart','JDend','ZPmed','ZPav','ZPsig','n']
    ZPtable['JDmidpoint'] = ZPtable.apply(lambda x: 2450000 + (x['JDstart'] + (x['JDend'] - x['JDstart'])/2), axis=1)
    #inconsistancy in the filter names
    ZPtable['filt'] = ZPtable['filt'].replace(['Rc'],'R')
    ZPtable['filt'] = ZPtable['filt'].replace(['Ic'],'I')
    ZPtable['filt'] = ZPtable['filt'].replace(['CO'],'CO+')
    ...
    if len(date) == 8:
        ...
        print(date)
        input('date format not recognised')
    startdateJD = sum(jdcal.gcal2jd(startdate.year, startdate.month, startdate.day)) +0.5
    ...
    for filt in filtlist:
        ...
        if len(ZPline) != 1:
            print(ZPline)
            print(filt)
            print('error finding the corresponding ZP')
            warning_flag = True

        datediff = (ZPline['JDmidpoint']-startdateJD).values[0]
        if abs(datediff) > 50:
            print("WARNING: update the ZP file, closest calibration midpoint at", datediff, "days")
            print(ZPfilt)
            warning_flag = True

        lines += f"{filt} {' '.join([str(int) for int in ZPparams.get(filt)])} {str(ZPline['ZPmed'].values[0])} {str(ZPline['ZPsig'].values[0])} {ZPline['JDmidpoint'].values[0]}\n"
        # print(line)
        # f.write(line + '\n')
    ...
    with open(output_path, 'w') as f:
        f.write(lines)
        f.write('\n# Generated by trap_reduction.generate_ZP_new()')
        f.write(f'\n# observatory: {obs}')
        f.write(f"\n# date: {date} (JD {startdateJD})")
        f.write("\n# see see calib08140914.dat for column info")
...
# import_perihelion(os.path.join(os.path.expanduser('~'), 'Documents/TRAPPIST/reduced_data/perihelions'), update=True)
