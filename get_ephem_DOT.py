...
if __name__ == "__main__":
    ...
    ### PARAMETERS ###
    comets = ['2022 U2','2022 E3','2020 V2','2019 U5', '81P'
    ...
    observatory = 'HCT' #TS, TN, HCT, Deva
    night = '2023-02-20'
    ...
    def import_eph(target): #function to import and format the ephemeris
        ...
        df = pd.DataFrame(data, columns=('date','dateJD', 'sol_marq', 'lun_marq', 'azi', 'elev', 'airmass', 'magex', 'rh','rh_rate','lun_ang', 'lun_ill'))
        df['date'] =  pd.to_datetime(df['date'], format=' %Y-%b-%d %H:%M')
        df = df.astype({'elev':'float', 'rh':'float'})
        print('rh:',df['rh'].mean(),'elev:',df['elev'].mean())
        #df = df.loc[df['rh']<=2]
    ...
    #initiate ephemeris
    eph = ephemeris()
    eph.parameters['QUANTITIES'] = '4,8,19,25'
    if observatory == 'TN':
        eph.parameters['CENTER'] = 'Z53@399'
    elif observatory == 'TS':
        eph.parameters['CENTER'] = 'I40@399'
    elif observatory == 'HCT':
        eph.parameters['CENTER'] = 'N50@399'
    ...
    try:
        ...
        print('could not find night nautical start, use night start instead')
    ...
    naut_end_time = df_moon.loc[df_moon['sol_marq'] == "A"][-1:]['date']
    naut_start_time = df_moon.loc[df_moon['sol_marq'] == "A"][:1]['date']
    dark_start_time = df_moon.loc[df_moon['sol_marq'] == " "][:1]['date']
    dark_end_time = df_moon.loc[df_moon['sol_marq'] == " "][-1:]['date'] 
    ...
    for comet in comets:
        ...
        if len(df.loc[df['elev']>0]) > 0:
            ...
            max_elev_date = df.loc[df['elev'].idxmax(), 'date']
            print(max_elev_date)
            print(naut_start_time.values[0])
            if max_elev_date < dark_start_time.values[0]:
                x_pos = naut_start_time.values[0] + (dark_start_time.values[0] - naut_start_time.values[0])/1.3
            elif max_elev_date > dark_end_time.values[0]:
                x_pos = naut_end_time.values[0] - (dark_start_time.values[0] - naut_start_time.values[0])/1.3
    ...
    plt.xlim(naut_start_time, naut_end_time)
    plt.ylim(0,90)
    plt.axvline(x=dark_start_time,
                color='black', ls='dotted', alpha=0.5)
    plt.axvline(x=dark_end_time,
                color='black', ls='dotted', alpha=0.5)
    plt.axhline(y=25, color='black', ls='dotted')
    ...
    ax2 = ax.twinx()
    ax2.set_ylim(0,90)
    plt.yticks(ticks=[10,20,30,40,50,60], labels=[5.6, 2.9, 2.0, 1.6, 1.3, 1.1])
